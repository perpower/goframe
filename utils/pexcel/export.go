...
func (e *gexport) SaveAsFile(f *excelize.File, fileName string) (err error) {
...
// fileName: string 文件名
func (e *gexport) SaveAsWeb(f *excelize.File, fileName string, w http.ResponseWriter) {
...
func (e *gexport) ExportExcel(sheetName string, cols []string, rows [][]interface{}) (*excelize.File, error) {
	f := excelize.NewFile()
	defer func() {
	...
	for rowIndex, row := range rows {
		for columnIndex, columnName := range columnNames {
			// 从第二行开始
			err := f.SetCellValue(sheetName, e.getColumnRowName(columnName, rowIndex+2), row[columnIndex])
			if err != nil {
				return nil, err
			}
		}
...
func (e *gexport) getColumnName(column, maxColumnRowNameLen int) []byte {
	const A = 'A'
...
func (e *gexport) getColumnRowName(columnName []byte, rowIndex int) (columnRowName string) {
	l := len(columnName)
