...
func ParseDuration(s string) (duration time.Duration, err error) {
	...
	if judge.IsNumeric(s) {
		...
		if err != nil {
			err = perrors.Newf(perrors.ERROR_3000.Code, `strconv.ParseInt failed for string "%s"`, err, s)
			return 0, err
		}
	...
	if len(match) == 3 {
		...
		if err != nil {
			err = perrors.Newf(perrors.ERROR_3000.Code, `strconv.ParseInt failed for string "%s"`, err, match[1])
			return 0, err
		}
		s = fmt.Sprintf(`%dh%s`, num*24, match[2])
		duration, err = time.ParseDuration(s)
		if err != nil {
			err = perrors.Newf(perrors.ERROR_3000.Code, `time.ParseDuration failed for string "%s"`, err, s)
		}
		return
	}
	duration, err = time.ParseDuration(s)
...
func StrToTime(str string, format ...string) (*Time, error) {
	...
	if match = timeRegex1.FindStringSubmatch(str); len(match) > 0 && match[1] != "" {
		...
		return nil, perrors.Newf(perrors.ERROR_3000.Code, `unsupported time converting for string "%s"`, nil, str)
	}
	...
	if match[6] != "" {
		...
		if len(zone) <= 6 {
			...
			if h > 24 || m > 59 || s > 59 {
				return nil, perrors.Newf(perrors.ERROR_3000.Code, `invalid zone string "%s"`, nil, match[6])
			}
			operation := match[5]
	...
	if month <= 0 || day <= 0 {
		return nil, perrors.Newf(perrors.ERROR_3000.Code, `invalid time string "%s"`, nil, str)
	}
...
func StrToTimeLayout(str string, layout string) (*Time, error) {
	if t, err := time.ParseInLocation(layout, str, time.Local); err == nil {
		...
		return nil, perrors.Newf(perrors.ERROR_3000.Code, `time.ParseInLocation failed for layout "%s" and value "%s"`, err, layout, str)
	}
}
